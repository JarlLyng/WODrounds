...
//  WODrounds
//
//  IAMJARL Design System tokens – mapped from tokens.json.
...
enum DesignTokens {
    ...
    // MARK: Spacing (tokens.json → tokens.spacing)
    enum Spacing {
    ...
    // MARK: Radius (tokens.json → tokens.radius)
    enum Radius {
        static let sm: CGFloat = 8
        static let md: CGFloat = 12
        static let lg: CGFloat = 16
    ...
    // MARK: Typography (tokens.json → brand.typography)
    enum Typography {
    ...
    // MARK: Color Tokens (tokens.json → tokens.colors)
    ...
    /// Scheme-based accessors (light + dark).
    enum Common {
        static func primary(_ scheme: ColorScheme) -> Color {
            color(light: ColorToken.Light.primary, dark: ColorToken.Dark.primary, scheme: scheme)
        }

        static func onPrimary(_ scheme: ColorScheme) -> Color {
            color(light: ColorToken.Light.onPrimary, dark: ColorToken.Dark.onPrimary, scheme: scheme)
        }

        enum Text {
            static func primary(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Text.primary, dark: ColorToken.Dark.Text.primary, scheme: scheme)
            }
            static func secondary(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Text.secondary, dark: ColorToken.Dark.Text.secondary, scheme: scheme)
            }
            static func tertiary(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Text.tertiary, dark: ColorToken.Dark.Text.tertiary, scheme: scheme)
            }
            static func inverse(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Text.inverse, dark: ColorToken.Dark.Text.inverse, scheme: scheme)
            }
        }

        enum Background {
            static func app(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Background.app, dark: ColorToken.Dark.Background.app, scheme: scheme)
            }
            static func muted(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Background.muted, dark: ColorToken.Dark.Background.muted, scheme: scheme)
            }
            static func card(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Background.card, dark: ColorToken.Dark.Background.card, scheme: scheme)
            }
        }

        enum Border {
            static func subtle(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Border.subtle, dark: ColorToken.Dark.Border.subtle, scheme: scheme)
            }
            static func `default`(_ scheme: ColorScheme) -> Color {
                color(light: ColorToken.Light.Border.default, dark: ColorToken.Dark.Border.default, scheme: scheme)
            }
        }
    }
}

// MARK: - Color parsing (hex + rgba)
